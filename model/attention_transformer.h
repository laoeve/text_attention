...
{
public:
    AttentionTransformer(int num_layers, int dim_model, int dim_ff, int heads,
                         int src_vocab, int tgt_vocab, int max_len)
    : src_vocab(src_vocab), dim_model(dim_model), max_len(max_len)
    {
        const std::string key = "generator.proj";
        encoder = new Encoder<T>(num_layers, dim_model, dim_ff, heads, max_len);
        decoder = new Decoder<T>(num_layers, dim_model, dim_ff, heads, max_len);

        /* Get weight and bias parameters */
        auto gen_w = new Tensor<T>(
                param_map[key+".weight"].pvals, 
                param_map[key+".weight"].pshape);
        t_tran(*gen_w);

        auto gen_b = new Tensor<T>(
                param_map[key+".bias"].pvals, 
                param_map[key+".bias"].pshape);
