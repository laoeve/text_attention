...
namespace text_attention {
    ...
    public:
        DecoderLayer(int num_layer, int dim_model, int dim_ff, int heads, int max_len, std::string str_key_layer) {
            maskedMultiheadAttention = new MultiheadAttention<T>(dim_model, heads, max_len, str_key_layer + "self_attn.linears.");
            multiheadAttention = new MultiheadAttention<T>(dim_model, heads, max_len, str_key_layer + "src_attn.linears.");
            positionwisefeedForward = new FeedForward<T>(dim_model, dim_ff, str_key_layer + "feed_forward.");
            preNorm1 = new PreNorm<T>(maskedMultiheadAttention, dim_model, str_key_layer + "sublayer.0.");
            preNorm2 = new PreNorm<T>(multiheadAttention, dim_model, str_key_layer + "sublayer.1.");
            preNorm3 = new PreNorm<T>(positionwisefeedForward, dim_model, str_key_layer + "sublayer.2.");
        ...
        void forward(const Tensor<T> &input, Tensor<T> &memory, Tensor<T> &output, Tensor<T> &tgt_mask, Tensor<T> &src_mask) {
            Tensor<T> tmp1{};
            Tensor<T> tmp2{};
